...
var exports=module.exports={
	...
	/*	asciidoc Table
		...
		adocTable(data,delimiter,multiple,options) {
			if(!delimiter) delimiter='\t';
			if(!options) options={};

			var	{data,columns,headers,options}=this.text2table(data,delimiter,multiple);
			var table=[];
			let row='|====';
	...
	/*	Box Character Table
		...
		boxTable(data,delimiter,multiple,options) {
			if(!delimiter) delimiter='\t';
			if(!options) options={};
			var	{data,columns,headers,options}=this.text2table(data,delimiter,multiple);
			var table=[];
	...
	/*	Github Flavoured Markdown Table
		...
		gfmTable(data,delimiter,multiple,options) {
			if(!delimiter) delimiter='\t';
			if(!options) options={};
			var	{data,columns,headers}=this.text2table(data,delimiter,multiple);
			var table=[];
	...
	/*	Read Raw Data into an Array
		...
		text2table(text,delimiter,multiple) {
			if(multiple) delimiter=new RegExp(delimiter+'+');
			var data,options;
			[data,options]=this.getData(text);
	...
	/*	Make Lists
		...
		/*	Headings & Paragraphs
			...
			makeHeadingsParagraphs: function(text,parms,eol) { // text,id,className) {
				parms=parms||{};
				//var [id,className]=this.idClass(parms);
		...
		/*	Links
			...
			addLink: function(parms) {
				...
				switch(parms.type) {
					case 'css':
						var link='<link rel="%s"%s type="text/css" href="%s" media="%s"%s>';
						var title = parms.title ? ' title="%s"'.sprintf(parms.title) : '';
						return link.sprintf(parms.rel||'stylesheet',title,parms.href||'',parms.media||'all',parms.xhtml?'/':'');
					case 'js':
						var link='<script type="text/javascript" src="%s"%s></script>';
					...
					case 'img':
						var src=this.attribute('src',parms.src,true);
						var alt=this.attribute('alt',parms.alt,true);
						var title=this.attribute('title',parms.title,true);
						var width=this.attribute('width',parms.width,true);
						var height=this.attribute('height',parms.height,true);
