		...
		makeHTMLStructure(text,parms,eol) {
			...
			function openElement(element,level,options) {
				...
				//	Attributes
					...
					var attributes=[];
					Object.keys(data.attributes).forEach(key => attributes.push(`${key}="${data.attributes[key]}"`));
					attributes = attributes.length ? ' %s'.sprintf(attributes.join(' ')) : '';
				level=level||0;
				if(html) level++;
				var tabs='\t'.repeat(level);
				return '%s<%s%s%s%s>'.sprintf(tabs,element,id,className,attributes);
			}
			...
			function parse(string) {
				...
				var data={
					...
					element: result[2],
					"void": result[3]=='/',
					inline: result[2]&&result[11]!==undefined?result[11]:undefined,
					id: result[5],
					"class": result[7],
					content: result[11],
					attributes: {}
				};
				if(result[9]) result[9].split('&').forEach(value=>{
					var split=value.split('=');
					if(split[1]) data.attributes[split[0]]=split[1];
					else data.attributes['name']=split[0];
				});
				return data;
			}
		},
